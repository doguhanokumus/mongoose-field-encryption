...
const fieldEncryption = function (schema, options) {
  ...
  function findHook(_next) {
    ...
    if (this.$useProjection) {
      // console.log(this.schema_tree);
      console.log('_conditions', this._conditions)
      Object.keys(this._conditions).forEach(key => {
        if (fieldsToEncrypt.indexOf(key) === -1) return;
        if (!this._conditions['$and']) this._conditions['$and'] = [];
        this._conditions['$and'].push({
          $or: [
            {
              $and: [
                { [key]: encrypt(this._conditions[key], secret(), saltGenerator) },
                { [`__enc_${key}`]: true },
              ]
            },
            {
              $and: [
                { [key]: this._conditions[key] },
                { [`__enc_${key}`]: false },
              ]
            }
          ]
      ...
      console.log('_conditions', JSON.stringify(this._conditions, null, 2))

      Object.keys(this._fields || Object.keys(schema.tree)).forEach(key => {
        if (fieldsToEncrypt.indexOf(key) === -1) return;
